...
impl System for DamageProcessingSystem {
    fn run(&self, world: &mut World) {
        let entities: Vec<_> = world.entities().cloned().collect();        for &entity in &entities {
            if world.has_ephemeral_component::<DamageEvent>(entity) {
                if let (Some(damage), Some(health)) = (
                    ...
                    if new_health == 0 {
                        world.add_ephemeral_component(entity, DeathEvent {
                            cause: damage_source,
                        }).ok();
                    }
                }
...
impl System for LoggingSystem {    fn after_run(&self, world: &World) {
        let entities: Vec<_> = world.entities().cloned().collect();

        for &entity in &entities {
            if world.has_ephemeral_component::<DeathEvent>(entity) {
                if let Some(death) = world.get_ephemeral_component::<DeathEvent>(entity) {
                    self.events.borrow_mut().push(format!("Entity died from {}", death.cause));
                }
            }
        }
    }
...
fn test_ephemeral_components_cross_system_communication() {
    ...
    world.add_component(entity1, Position { x: 60.0, y: 10.0 }).unwrap(); // Will take damage
    world.add_component(entity2, Position { x: 30.0, y: 20.0 }).unwrap(); // Won't take damage
    world.add_component(entity3, Position { x: 70.0, y: 30.0 }).unwrap(); // Will take damage

    world.add_component(entity1, Health { current: 100, max: 100 }).unwrap();
    world.add_component(entity2, Health { current: 100, max: 100 }).unwrap();
    world.add_component(entity3, Health { current: 5, max: 100 }).unwrap(); // Low health
    ...
    scheduler.add_system(LoggingSystem::new(events.clone())).unwrap();
...
fn test_ephemeral_components_persist_across_system_phases() {
        ...
        fn run(&self, world: &mut World) {
            for entity in world.entities().cloned().collect::<Vec<_>>() {
                if world.has_component::<Position>(entity) {
                    world.add_ephemeral_component(entity, MovementEvent {
                        dx: 5.0,
                        dy: 3.0,
                    }).ok();
                }
    ...
    impl System for MovementProcessorSystem {
        fn run(&self, world: &mut World) {
            for entity in world.entities().cloned().collect::<Vec<_>>() {
                if world.has_ephemeral_component::<MovementEvent>(entity) {
                    self.events.borrow_mut().push("Movement event found in run phase".to_string());
        ...
        fn after_run(&self, world: &World) {
            for entity in world.entities().cloned().collect::<Vec<_>>() {
                if world.has_ephemeral_component::<MovementEvent>(entity) {
                    self.events.borrow_mut().push("Movement event still exists in after_run phase".to_string());
                }
            }
        }
    }
    ...
    scheduler.add_system(MovementProcessorSystem::new(events.clone())).unwrap();
    scheduler.build().unwrap();
    ...
    assert_eq!(logged_events[1], "Movement event still exists in after_run phase");
...
fn test_ephemeral_components_with_multiple_ticks() {
    ...
    world.replace_component(entity, Health { current: 60, max: 100 });
...
fn test_ephemeral_components_with_entity_deletion_in_systems() {
    ...
    world.add_component(entity1, Health { current: 100, max: 100 }).unwrap();
    world.add_component(entity2, Health { current: 5, max: 100 }).unwrap(); // Will die
    world.add_component(entity3, Health { current: 50, max: 100 }).unwrap();
...
fn test_ephemeral_components_performance_with_many_entities() {
    ...
    for &entity in entities.iter().take(10) { // Check first 10
        let health = world.get_component::<Health>(entity).unwrap();
